 ...
 */
class DefaultAdapter implements ImageFinderInterface {
  /**
   * Default minimum confidence threshold for matches
   */
  readonly DEFAULT_CONFIDENCE = 0.8;
   ...
   */
   async imageToRawBuffer(image: Image): Promise<Uint8Array> {
    try {
   ...
   */
   calculateMean(buffer: Uint8Array): number {
    let sum = 0;
   ...
   */
   calculateStdDev(buffer: Uint8Array, mean: number): number {
    let sumSq = 0;
    for (let i = 0; i < buffer.length; i++) {
